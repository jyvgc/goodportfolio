...
export default function HomePage() {
  ...
  const [categories, setCategories] = useState<string[]>(["ALL","웹툰","게임아트","캐릭터","배경","UI/UX","3D"]);
...
setWorks(
  ...
  useEffect(() => {
    if (heroType==="slide" && gridImages.length>1) {
      slideTimer.current = setInterval(() => setSlideIndex((p) => (p+1)%gridImages.length), 3000);
  ...
  const jsonLd = {"@context":"https://schema.org","@type":"WebSite","name":"GoodPortfolio","url":"https://goodportfolio-five.vercel.app","description":"구미대학교 웹툰·게임콘텐츠 학생 포트폴리오 플랫폼"};
      ...
      <section style={{ minHeight:"100vh",paddingTop:80,position:"relative",overflow:"hidden",background:"linear-gradient(135deg,#0a0a0f 0%,#1a1a2e 50%,#16213e 100%)",opacity:isLoaded?1:0,transition:"opacity 0.3s ease-in" }}>
        ...
        <div style={{ maxWidth:mw,margin:"0 auto",padding:"60px 24px 40px" }}>
          <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:32 }}>
            ...
            <span style={{ color:"#818cf8",fontSize:11,fontWeight:600,letterSpacing:"0.2em",textTransform:"uppercase" }}>{heroBadgeText}</span>
          </div>
      ...
      {notices.length>0 && (
        <section style={{ background:"#111118",borderTop:"1px solid #1a1a24",borderBottom:"1px solid #1a1a24" }}>
          <div style={{ maxWidth:mw,margin:"0 auto",padding:"24px 24px" }}>
            ...
            {notices.map((n) => (
              <div key={n.id} style={{ display:"flex",alignItems:"flex-start",gap:10,marginBottom:8 }}>
