...
import OfferModal from "@/components/portfolio/OfferModal";
...
export default function PortfolioPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#f0f0ff" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                ...
                {currentUser?.role === "company" && currentUser?.isApproved && (
                  <button onClick={() => setShowOfferModal(true)} style={{ background: "#6366f1", color: "white", padding: "8px 20px", borderRadius: 8, fontWeight: 600, fontSize: 14, border: "none", cursor: "pointer" }}>💼 채용 제안하기</button>
                )}
              </div>
            </div>
                ...
                <Link key={w.id} href={`/work/${w.id}`} style={{ textDecoration: "none" }}>
                  <div style={{ background: "#111118", border: "1px solid #2e2e3f", borderRadius: 16, overflow: "hidden", cursor: "pointer", transition: "all 0.3s" }}
                    ...
                    <div style={{ aspectRatio: "1", background: "#1a1a24", overflow: "hidden", position: "relative" }}>
                      {w.images?.[0] ? <img src={w.images[0]} alt={w.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36 }}>🎨</div>}
                      ...
                      {(w.images?.length ?? 0) > 1 && (
                        <div style={{ position: "absolute", bottom: 8, right: 8, background: "rgba(0,0,0,0.7)", color: "white", fontSize: 10, padding: "2px 8px", borderRadius: 999 }}>+{(w.images?.length ?? 1) - 1}</div>
                      )}
                    </div>
      ...
      {showOfferModal && <OfferModal toStudentUid={uid} studentName={userInfo.displayName || "학생"} onClose={() => setShowOfferModal(false)} />}
      <Footer />
    </div>
  );
}
