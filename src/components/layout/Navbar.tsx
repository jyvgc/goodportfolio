...
export default function Navbar() {
  ...
  const dashboardPath = userDoc?.role==="admin" ? "/admin" : userDoc?.role==="company" ? "/dashboard/company" : "/dashboard/student";
  const dashboardLabel = userDoc?.role==="admin" ? "🔐 관리자" : "대시보드";
  const passwordPath = userDoc?.role==="company" ? "/dashboard/company/password" : "/dashboard/student/password";
  ...
  const menuItem = (href: string, label: string, onClick?: ()=>void) => (
    <Link href={href} onClick={() => { setProfileMenuOpen(false); onClick?.(); }}
  ...
  return (
    <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:50, transition:"all 0.3s", background:scrolled||menuOpen?"rgba(10,10,15,0.97)":"transparent", backdropFilter:scrolled?"blur(12px)":"none", borderBottom:scrolled?"1px solid #2e2e3f":"none" }}>
